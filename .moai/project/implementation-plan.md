...
| 0x06 | REG_TLINE | 16 | R/W | 라인 타임 (10ns 단위, AFE별 최소값: AD71124≥2200(22µs), AD71143≥6000(60µs), AFE2256≥5120(51.2µs)) |
...
## Verification Plan (SPEC-FPD-SIM-001 기반)

### 3단계 검증 마일스톤

| 단계 | 목표 | 완료 기준 |
|------|------|-----------|
| V1 | 개별 모듈 smoke test + CSR (Register) 검증 | 각 SPEC AC의 기본 경로 통과 |
| V2 | FSM 시나리오 + 스트레스 테스트 + CDC 검증 | 경계 조건, 에러 주입, CDC 안전성 확인 |
| V3 | 전체 시스템 regression + coverage 95%+ | 하드웨어 조합 C1-C7 전수 검증 완료 |

### 검증 도구 스택

| 도구 | 역할 | 라이선스 |
|------|------|---------|
| C++17 (gcc/MSVC) | 골든 모델 (알고리즘 정확성) | Free |
| GoogleTest | C++ 단위 테스트 | Open-source |
| cocotb 2.0 | Python 테스트벤치 | Open-source |
| Verilator | RTL→C++ 사이클 정확 비교 | Open-source |
| Vivado xsim | Xilinx 프리미티브 검증 | Free (Vivado) |

### Coverage 목표

- C++ 골든 모델: 90%+ 라인 커버리지 (R-SIM-036)
- RTL 코드 커버리지: 85%+ (TRUST 5 기준)
- Functional 커버리지: 하드웨어 조합 C1-C7 전수 검증
...
| 표준 | 범위 | 비고 |
|------|------|------|
| IEC 62304 Class B | 의료기기 소프트웨어 수명주기 | FPGA 포함 여부 사전 협의 필요 |
| IEC 60601-1 | 의료 전기 장비 안전 | 전원 시퀀싱, 비상 차단 요구사항 관련 |
| FDA Cybersecurity Guidance 2024 | programmable logic 포함 기기 | FPGA 업데이트 메커니즘 설계 필요 |

### 필요 문서 (향후 작성)

- Verification & Validation Plan
- 요구사항 추적성 매트릭스 (DO-254 참조)
- FPGA 고장 모드 분석 (Risk Analysis, FMEA)
- FPGA 현장 업데이트 메커니즘 설계
...
- v3.2: 교차검증 반영 — REG_NRESET(0x16) 레지스터 추가, REG_TLINE AFE별 최소값 명세 업데이트, Verification Plan 섹션 추가 (3단계 마일스톤 + 도구 스택 + Coverage 목표), 규제 준수 참고사항 섹션 추가 (IEC 62304/60601-1/FDA)
