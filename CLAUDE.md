...
## Project: X-ray Flat Panel Detector FPGA Design

### Domain Overview

This repository contains FPGA design documentation and (future) SystemVerilog RTL for an X-ray Flat Panel Detector (FPD) control system. The FPGA drives a-Si TFT panels via Gate IC and AFE/ROIC components, and interfaces with an MCU via SPI.

### Hardware Combination Matrix (C1–C7)

All designs target 7 hardware combinations:

| ID | Panel | Gate IC | AFE/ROIC | Use Case |
|----|-------|---------|----------|----------|
| C1 | R1717 (17×17") | NV1047 | AD71124 | Standard still imaging |
| C2 | R1717 | NV1047 | AD71143 | Low-power / mobile |
| C3 | R1717 | NV1047 | AFE2256 | High-quality (low noise, CIC) |
| C4 | R1714 (17×14") | NV1047 | AD71124 | Non-square panel |
| C5 | R1714 | NV1047 | AFE2256 | High-quality 17×14 |
| C6 | X239AW1-102 (43×43cm) | NT39565D ×6 | AD71124 ×12 | Large panel, multi-AFE |
| C7 | X239AW1-102 | NT39565D ×6 | AFE2256 ×12 | Large panel, high-quality |

### Module Hierarchy

```
fpga_top_cX.sv          (per-combination, pin mapping only)
├── spi_slave_if.sv     [common] MCU↔FPGA SPI register R/W
├── clk_rst_mgr.sv      [common, param: ACLK_HZ/MCLK_HZ]
├── reg_bank.sv         [common] 32-register config/status file
├── panel_ctrl_fsm.sv   [common] Main FSM: IDLE→RESET→INTEGRATE→READOUT→DONE
│   ├── gate_ic_driver  [select: gate_nv1047 / gate_nt39565d]
│   │   └── row_scan_eng.sv  [common]
│   ├── afe_ctrl_if     [select: afe_ad711xx / afe_afe2256]
│   │   └── line_data_rx.sv  [common, param: ADI_MODE/TI_MODE]
│   │       └── line_buf_ram.sv  [common]
│   └── prot_mon.sv     [common] over-exposure timeout, error flags
├── data_out_mux.sv     [common]
└── mcu_data_if.sv      [common]
```

**Key design rules:**
- All timing parameters come from `reg_bank` at runtime — never hardcode timing values
- Hardware combination differences are isolated to `gate_ic_driver` + `afe_ctrl_if` + pin mapping in `fpga_top`
- `panel_ctrl_fsm` passes only timing signals to gate/AFE modules; data path goes through `line_data_rx`

### FSM Operating Modes (REG_MODE[2:0])

| Value | Mode | Description |
|-------|------|-------------|
| 000 | STATIC | Single frame acquisition |
| 001 | CONTINUOUS | Auto-repeat DONE→RESET |
| 010 | TRIGGERED | Wait for X-ray external trigger |
| 011 | DARK_FRAME | Gate off, AFE readout only (offset calibration) |
| 100 | RESET_ONLY | Panel reset only |

### Key Register Map

Register access via MCU SPI (addresses 0x00–0x1F). Critical registers:
- `REG_COMBO (0x03)`: Hardware combination select (C1–C7)
- `REG_TLINE (0x06)`: Line time in 10ns units (min 2200 for AD71124, 6000 for AD71143)
- `REG_TGATE_ON (0x09)`: Gate ON pulse width
- `REG_CIC_EN (0x0E)`: AFE2256 Charge Injection Compensation enable

### Documentation Structure

- `docs/fpga-design/fpga_module_architecture.md` — Complete module spec and register map
- `docs/fpga-design/fpga-xray-detector-driving-algorithm.pplx.md` — Driving algorithm design spec (FPD-FPGA-DRV-001)
- `docs/fpga-design/fpga-radiography-static-imaging.pplx.md` — Static radiography sequences
- `docs/fpga-design/fpga-panel-power-settings.pplx.md` — Power sequencing
